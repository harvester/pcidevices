...
func (h *Handler) SetupSRIOVGPUDevices(baremetalGPU bool) error {
	sriovGPUDevices, err := gpuhelper.IdentifySRIOVGPU(h.options, h.nodeName)
	if err != nil {
		return err
	}
	return h.reconcileSRIOVGPUSetup(sriovGPUDevices, baremetalGPU)
}

// reconcileSRIOVGPUSetup runs the core logic to reconcile the k8s view of node with actual state on the node
func (h *Handler) reconcileSRIOVGPUSetup(sriovGPUDevices []*v1beta1.SRIOVGPUDevice, baremetalGPU bool) error {
	// cleanup devices if baremetalGPU is set to true
	if baremetalGPU {
