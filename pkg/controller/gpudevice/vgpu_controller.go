...
func (h *Handler) SetupVGPUDevices(baremetalGPU bool) error {
	vGPUDevices, err := gpuhelper.IdentifyVGPU(h.options, h.nodeName)
	if err != nil {
		return fmt.Errorf("error identifying vgpu devices: %v", err)
	}
	return h.reconcileVGPUSetup(vGPUDevices, baremetalGPU)
}

func (h *Handler) reconcileVGPUSetup(vGPUDevices []*v1beta1.VGPUDevice, baremetalGPU bool) error {
	if baremetalGPU {
