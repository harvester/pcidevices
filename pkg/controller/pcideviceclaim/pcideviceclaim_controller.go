...
func (h *Handler) OnRemove(_ string, pdc *v1beta1.PCIDeviceClaim) (*v1beta1.PCIDeviceClaim, error) {
	...
	if val, ok := pdc.Annotations[v1beta1.PCIDeviceOverrideResourceName]; ok {
		resourceName = val
	}
...
func (h *Handler) reconcilePCIDeviceClaims(_ string, pdc *v1beta1.PCIDeviceClaim) (*v1beta1.PCIDeviceClaim, error) {
	...
	// for certain devices like nvidia vGPUs on kernel 6.8+ the vGPU is passed
	// through as a normal PCIdevice but we need to skip binding to vfio driver
	if !skipDeviceBindingOp(pdc) {
		...
		// because device is not going to be bound to vfio
		// this is likely a vgpu device
		// so we are going to check resourceName generated by vgpu controller from pdc annotation
		// and pdc as enabled
	...
	if dp == nil {
		...
		dp, err = h.createDevicePlugin(pds, pdc, resourceName)
		if err != nil {
			return pdc, err
		}
...
func skipDeviceBindingOp(pd *v1beta1.PCIDeviceClaim) bool {
	if pd.Annotations == nil {
		return false
	}

	if _, ok := pd.Annotations[v1beta1.SkipVFIOBindingAnnotationKey]; ok {
		return true
	}
