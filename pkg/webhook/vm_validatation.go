...
func NewDeviceHostValidation(pciCache v1beta1.PCIDeviceCache, vgpuCache v1beta1.VGPUDeviceCache) types.Validator {
...
func (vmValidator *vmDeviceHostValidator) validateHostDevices(vmObj *kubevirtv1.VirtualMachine) error {
	for _, hostDevice := range vmObj.Spec.Template.Spec.Domain.Devices.HostDevices {
		...
		if !foundInPCI {
			return fmt.Errorf("hostdevice %s: resource name %s not found in pcidevice cache", hostDevice.Name, hostDevice.DeviceName)
		}
	}
	return nil
}
