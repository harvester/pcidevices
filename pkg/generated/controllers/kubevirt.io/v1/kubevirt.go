...
// RegisterKubeVirtStatusHandler configures a KubeVirtController to execute a KubeVirtStatusHandler for every events observed.
...
// RegisterKubeVirtGeneratingHandler configures a KubeVirtController to execute a KubeVirtGeneratingHandler for every events observed, passing the returned objects to the provided apply.Apply.
// If a non-empty condition is provided, it will be updated in the status conditions for every handler execution
...
// Handle executes the configured KubeVirtGeneratingHandler and pass the resulting objects to apply.Apply, finally returning the new status of the resource
...
func (a *kubeVirtGeneratingHandler) isNewResourceVersion(obj *v1.KubeVirt) bool {
	if !a.opts.UniqueApplyForResourceVersion {
		return true
	}
...
// Only used if UniqueApplyForResourceVersion is set in generic.GeneratingHandlerOptions
func (a *kubeVirtGeneratingHandler) storeResourceVersion(obj *v1.KubeVirt) {
	if !a.opts.UniqueApplyForResourceVersion {
		return
	}
